...
**State:** Input (Excel file path, trigger date, recipients) → Output (email content, subject, metadata)

**Data Flow:** Excel file → Crew 1 (categorization) → Crew 2 (email generation) → Save to `data/output/`
...
- Subject: "TBS Price Change Summary – Effective [Date]" (using effective_date from filename)
